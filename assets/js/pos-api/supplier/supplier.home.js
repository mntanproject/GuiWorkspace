...
var displayData = function (offset, limit) {
    document.querySelectorAll('.supplier-data').forEach(e => e.remove());
    let url = 'http://localhost:9000/api/supplier/paginator/'
    getData(url + 'offset=' + offset + '&limit=' + limit, generateSupplierHtml)
}


function refreshPage(numRows) {
    var pagination = new MnPagination(data.totalsize, numRows, 1, 3, 'paginatorTop', displayData)
    generatePagination(pagination)
    displayData(pagination.currentPage, pagination.rowsPerPage)
...
function supplierHome(data) {
    ...
    var pagination = new MnPagination(data.totalsize, 5, 1, 3, 'paginatorTop', displayData)
    generatePagination(pagination)
    displayData(pagination.currentPage, pagination.rowsPerPage)
                    ...
                    <a class="accordion-button collapsed btn" data-bs-toggle="collapse" href="#flush-collapse' + index + '" role="button" aria-expanded="false" aria-controls="#flush-collapse' + index + '">\
                        <div class="container-fluid p-0 m-0">\
                            <dl class="row m-0">\
                                ...
                                <dd class="col-9 col-md-4 text-left border-right-md">' + supp.company + '</dd>\
