...
function generateHtml(item, index, arr) {
    ...
    accCollapseContentContainer.innerHTML =
        '<dt class="col-2 small p-0">Purchase price</dt><dd class="col-10 small">' +
        nullValueToEmpty(mn.purchasePrice) +
        '</dd><dt class="col-2 small p-0">Selling price</dt><dd class="col-10 small">' +
        nullValueToEmpty(mn.sellingPrice) +
        '</dd><dt class="col-2 small p-0">Notes</dt><dd class="col-10 small">' +
        nullValueToEmpty(mn.notes) +
        "</dd>";
    ...
    accCollapseContentButtonEdit.addEventListener("click", function () {
        window.location.href = "item_edit.html?id=" + mn.id;
    });
...
function search() {
    var url =
        MnItem.sizeUrl + "search=" + document.getElementById("search").value;
    var numRows = document.querySelector('input[name = "btnradiorows"]:checked')
        .value;
    paginationInit.rowsPerPage = numRows;
    paginationInit.currentPage = 1;
