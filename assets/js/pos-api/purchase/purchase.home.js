    ...
    .then(function (result) {
      if (result.length == 0) {
        ...
        paginationSummary.innerHTML =
          "Showing " + (numOffset + 1) + " to " + (numOffset + numRecordIn);
  ...
  accCollapseContentButtonDelete.addEventListener("click", function () {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        Swal.fire({ icon: "success", title: "Deleted" }).then(function () {
          // Redirect the user
...
function search() {
  var url =
    MnPurchase.sizeUrl + "search=" + document.getElementById("search").value;
  var numRows = document.querySelector('input[name = "btnradiorows"]:checked')
    .value;
  paginationInit.rowsPerPage = numRows;
  paginationInit.currentPage = 1;
